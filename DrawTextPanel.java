...
public class DrawTextPanel extends JPanel  {
	...
	private ArrayList<DrawTextItem> theStringArr;  // change to an ArrayList<DrawTextItem> !
	...
	private class Canvas extends JPanel {
		...
		protected void paintComponent(Graphics g) {
			...
			if(theStringArr != null) {
				for(DrawTextItem str : theStringArr) {
					str.draw(g);
				}
			}
...
//		s.setBackgroundTransparency(0.7);  // Default is 0, meaning background is not transparent.
		...
		if(theStringArr == null) {
			theStringArr = new ArrayList<DrawTextItem>();
		}
		theStringArr.add(s);
//		theStringArr = s;  // Set this string as the ONLY string to be drawn on the canvas!
	...
	private void doMenuCommand(String command) {
		if (command.equals("Save...")) { // save all the string info to a file
			File stringFile = fileChooser.getOutputFile(this, "Select Text File", "Strings text.txt");
			try {
				...
				if(theStringArr != null) {
					...
					for(DrawTextItem str : theStringArr) {
						...
						writer.println(str.getString());
						
						writer.println(str.getTextColor().getRed());
						writer.println(str.getTextColor().getGreen());
						writer.println(str.getTextColor().getBlue());
						writer.println(str.getTextTransparency());
						
						writer.println(str.getX());
						writer.println(str.getY());
...
//			JOptionPane.showMessageDialog(this, "Sorry, the Save command is not implemented.");
			...
			File readFile = fileChooser.getInputFile(this, "Select a File");
			
			try {
				Scanner reader = new Scanner(readFile);
				
				if(!reader.nextLine().equals("BackgroundColor")) {
					return;
				}
				
				Color bckground = new Color(reader.nextInt(),reader.nextInt(), reader.nextInt());
				
				reader.nextLine();
				reader.nextLine();
				reader.nextLine();
				
				ArrayList<DrawTextItem> strArr = new ArrayList<>();
				DrawTextItem itm;
				
				while(reader.hasNext() && (reader.nextLine()).equals("theString")) {
					itm = new DrawTextItem(reader.nextLine());
					itm.setTextColor(new Color(reader.nextInt(), reader.nextInt(), reader.nextInt()));
					itm.setTextTransparency(reader.nextDouble());
					itm.setX(reader.nextInt());
					itm.setY(reader.nextInt());
				...
				canvas.setBackground(bckground);
				theStringArr = strArr;
				reader.close();
			...
			theStringArr = null;   // Remove the ONLY string from the canvas.
			undoMenuItem.setEnabled(false);
			canvas.repaint();
		}
		else if (command.equals("Remove Item")) { // remove the most recently added string
			theStringArr.remove(theStringArr.size()-1);   // Remove the ONLY string from the canvas.
			undoMenuItem.setEnabled(false);
			canvas.repaint();
		}
